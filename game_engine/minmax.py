...
class min_max():
    ...
    def get_successors(self, dict_board, position):   
        ...
        jump_possibles = self.check_jump(dict_board, position, [0, start_space])
    ...
    def check_jump(self, dict_board, start_pos, prev_moves):
        ...
        if friend in start_piece:
            ...
            if start_pos_x < 6 and start_pos_y > 1:
                jump_possibles.append(self.jump_possible(start_pos, start_pos_x, start_pos_y,[1, 2],[-1, -2], enemy, enemy_king, adjust, dict_board, prev_moves))
    ...
    def jump_possible(self, start_pos, start_pos_x, start_pos_y, x_adjust, y_adjust, enemy, enemy_king, adjust, dict_board, prev_moves):
        gain = 0
        
        start_position = start_pos[0]
        check_pos = 'space'+str(start_pos_x+x_adjust[0])+str(start_pos_y+y_adjust[0])
        if enemy in dict_board[check_pos]:
            if enemy_king in dict_board[check_pos]:
                gain += 8
            else:
                gain += 3
            end_pos = 'space'+str(start_pos_x+x_adjust[1])+str(start_pos_y+y_adjust[1])
